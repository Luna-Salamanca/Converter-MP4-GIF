...
const TimeInput = ({ 
  ...
  if (isEditing) {
    return (
        <input 
            className={cn("bg-transparent border-b border-primary text-primary font-medium w-[70px] outline-none text-center p-0 h-auto leading-none", className)}
            value={inputValue}
  ...
  return (
    <span 
        className={cn("text-primary font-medium cursor-text hover:bg-white/10 px-1 rounded transition-colors", className)}
...
export function VideoTimeline({ 
  ...
  const [isDragging, setIsDragging] = useState<"left" | "right" | "scrub" | null>(null);
    ...
    <div className="space-y-4 bg-card/50 border border-border/50 rounded-xl p-4 backdrop-blur-sm">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="font-mono text-sm bg-black/40 px-3 py-1.5 rounded-md border border-white/10 shadow-inner flex items-center gap-2 tabular-nums">
          <span className="text-muted-foreground text-[10px] uppercase tracking-wider">Timecode</span> 
          ...
          <span className="text-muted-foreground">{formatTime(trimRange[1])}</span>
        </div>
      </div>
        ...
        <div className="absolute inset-0 bg-[#0A0A0A] rounded-lg overflow-hidden border border-white/5 shadow-inner">
            ...
            <div className="absolute inset-0 top-8 bottom-0 flex pointer-events-none bg-neutral-900/50">
               ...
               <div className="w-full h-full opacity-20" 
                    style={{
                      backgroundImage: 'repeating-linear-gradient(90deg, transparent 0, transparent 1px, rgba(255,255,255,0.05) 1px, rgba(255,255,255,0.05) 100px)'
                    }}
               />
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <div className="flex items-center gap-2">
            ...
            <span>Selection Duration: <span className="text-foreground font-medium">{formatTime(trimRange[1] - trimRange[0])}</span></span>
        </div>
