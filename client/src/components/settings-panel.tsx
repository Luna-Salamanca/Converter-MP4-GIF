...
export function SettingsPanel({
  ...
  useEffect(() => {
    ...
    const effectiveFps = fastMode ? Math.min(fps[0], 15) : fps[0];
  ...
  const handleExport = async () => {
    ...
    const duration = Math.max(0.1, trimRange[1] - trimRange[0]);
    const effectiveFps = fastMode ? Math.min(fps[0], 15) : fps[0];
    const numFrames = Math.max(1, Math.floor(duration * effectiveFps));
    const delay = 1000 / effectiveFps; // Delay in ms
    ...
    try {
      ...
      for (let i = 0; i < numFrames; i++) {
        const time = trimRange[0] + i / effectiveFps;
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-8">
        ...
        <div className="space-y-4">
          ...
          <Slider
            ...
            min={5}
            step={1}
            disabled={fastMode} // Optional: disable slider in fast mode if we force 15
