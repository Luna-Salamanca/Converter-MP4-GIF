...
interface PreviewPlayerProps {
  ...
  onCropChange?: (value: { x: number; y: number; width: number; height: number }) => void;
}
      ...
      >
        ...
        <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-30">
             <div className="bg-black/50 backdrop-blur text-xs px-2 py-1 rounded text-white border border-white/10 flex items-center">
                Original: {videoDimensions.width > 0 ? `${videoDimensions.width}x${videoDimensions.height}` : "Loading..."}
             </div>
             <Button 
               size="icon"
               variant="ghost"
               className={cn(
                 ...
                 isCropping && "bg-primary text-primary-foreground hover:bg-primary/90"
               )}
